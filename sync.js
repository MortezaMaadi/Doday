...
window.Sync = (() => {
  ...
  const WORKER_URL =
    'https://71fe94a5-doday.mortezamoadi.workers.dev';
  ...
  async function pushData(dataObj) {
    ...
    const userId = currentUser?.id || 'morteza';

    const result = await workerRequest(
      `/api/data?user=${encodeURIComponent(userId)}`,
      {
        method: 'PUT',
        body: JSON.stringify({
          data: dataObj
